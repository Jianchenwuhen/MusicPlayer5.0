...
public class Findmusic {
    public List<Music> getmusics(ContentResolver contentResolver) {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            ...
            cursor.moveToNext();
            String title = cursor.getString((cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE)));//音乐标题
            ...
            String url = cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA));	//文件路径
            String album = cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM)); //唱片图片
            long album_id = cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.ALBUM_ID)); //唱片图片ID
